...
{
    public partial class frm_Area_Report : Form
    {
        ReportDocument rd = new ReportDocument();

        public frm_Area_Report()
        {
            InitializeComponent();
        }
        ...
        private void frm_Area_Report_Load(object sender, EventArgs e)
        {
            string sqlStr = "Select Area_Id,Area_Name,Km,Bike_Km_Price,Car_Km_Price from Area_Detail";

            SqlDataAdapter da = new SqlDataAdapter(sqlStr, Con);

            DataSet ds = new DataSet();

            da.Fill(ds, "Area_Detail");

            Area_CrystalReport cr = new Area_CrystalReport();
            cr.SetDataSource(ds);
