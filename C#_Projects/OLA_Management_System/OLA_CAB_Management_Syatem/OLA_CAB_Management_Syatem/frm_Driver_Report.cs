...
{
    public partial class frm_Driver_Report : Form
    {
        ReportDocument rd = new ReportDocument();

        public frm_Driver_Report()
        {
            InitializeComponent();
        }
        ...
        private void frm_Driver_Report_Load(object sender, EventArgs e)
        {
            string sqlStr = "Select Driver_Id,Driver_Name,Mobile_No,Licence_No from Driver_Details";

            SqlDataAdapter da = new SqlDataAdapter(sqlStr, Con);

            DataSet ds = new DataSet();

            da.Fill(ds, "Driver_Details");

            Driver_CrystalReport cr = new Driver_CrystalReport();
            cr.SetDataSource(ds);
